(function(root){
  ...
  function render(spec,context={}){
    ...
    const RB=root.VAAKRevisionBlock,mk=(fields,html)=>RB&&RB.changed(spec,fields)?`<span class="rev-mark">${html}</span>`:html;
    ...
    return `<article class="spec-preview hpg-technical-sheet notranslate" translate="no" lang="en">
      <div class="hpg-ts-content">
        ...
        <table class="hpg-ts-project-table"><tbody>
          <tr><th>PROJECT</th><td>${esc(project.name||'')}</td><th>CATEGORY</th><td>${mk(['category'],esc(spec.category||''))}</td></tr>
          <tr><th>PROJECT #</th><td>${esc(project.code||'')}</td><th>VENDOR /<br>SOURCE</th><td>${mk(['vendorSource'],esc(vendor))}</td></tr>
          <tr><th>CLIENT</th><td>${esc(project.legal||project.contact||'')}</td><th>ISSUE DATE</th><td>${esc(issueDate)}</td></tr>
          <tr><th>AREA</th><td>${mk(['area'],esc(spec.area||''))}</td><th>STATUS</th><td>${mk(['specStatus'],esc(status))}</td></tr>
        </tbody></table>
        ...
        <section class="hpg-ts-specification">
          ...
          <div class="hpg-ts-spec-grid">
            <table><tbody>
              <tr><th>DESCRIPTION</th><td>${mk(['name'],esc(model))}</td></tr>
              <tr><th>PRODUCT CODE</th><td>${mk(['productCode'],esc(spec.productCode||''))}</td></tr>
              <tr><th>NOTES</th><td>${mk(['reference'],esc(reference))}</td></tr>
              <tr><th>SIZE</th><td>${mk(['size'],esc(spec.size||''))}</td></tr>
              <tr><th>FINISH / COLOR</th><td>${mk(['color'],esc(finish))}</td></tr>
              <tr><th>QTY.</th><td>${mk(['quantity','unit'],esc(qty))}</td></tr>
            </tbody></table>
        ...
        <section class="hpg-ts-copy hpg-ts-description">${(globalThis.VAAKRevisionBlock&&globalThis.VAAKRevisionBlock.html(spec))||''}<h2>DESCRIPTION</h2>${RB&&RB.changed(spec,['description'])?`<p><span class="rev-mark">${esc(valueOrBlank(description))}</span></p>`:paragraph(description)}</section>
        <table class="hpg-ts-item-table"><thead><tr><th>BY</th><th>QTY.</th><th>ITEM</th><th>ITEM #</th></tr></thead><tbody><tr><td>HPG</td><td>${mk(['quantity','unit'],esc(qty))}</td><td>${mk(['name'],esc(model))}</td><td>${mk(['code'],esc(itemNumber))}</td></tr></tbody></table>
      </div>
