    ...
    selected.forEach(order=>{
      ...
      items.forEach((item,itemIndex)=>{
        ...
        records.push([
          ...
          textEntry(item.reference||item.code||item.specCode||(item.specSnapshot&&item.specSnapshot.code)||codigoDeSpec(specs,item.specId)||`${order.number||'PO'}-${String(itemIndex+1).padStart(3,'0')}`,alt(7,8)),
          ...
          textEntry(item.unit||item.unitMeasure||(item.specSnapshot&&item.specSnapshot.unit)||unidadDeSpec(specs,item.specId)||'ea',alt(9,10)),
  ...
  const invoices = async (project,orders,opciones={}) => {
    ...
    const records=selected.map((invoice,index)=>{
      ...
      const fila=[
        ...
        numberEntry(moneyValue(order?.amountValue||order?.amount),alt(11,12)),
        ...
        // Los comentarios van siempre en INVOICE NOTE (pedido de Datnya, 18-sep-2026); HPG COMMENTS queda libre.
        textEntry([invoice.invoiceNote||invoice.paymentComments||invoice.hpgComments||invoice.requestDetail||'',invoice.paymentExtras?'Other payment lines: '+invoice.paymentExtras:''].filter(Boolean).join(' · '),alt(3,4)),
        textEntry('',alt(3,4)),
