(function(root,factory){
  ...
  function render(project={},invoice={}){
    ...
    return `<article class="payment-request-preview hpg-payment-request notranslate" translate="no" lang="en">
      ...
      <section class="pr-doc-section pr-entry-section"><h2>INVOICE ENTRY DETAILS</h2><div class="pr-entry-table"><div class="pr-entry-head"><span>INVOICE #</span><span>PO REF #</span><span>INV. DATE</span><span>DUE DATE</span><span>PAYMENT<br>TERMS</span><span>CURR</span><span>TOTAL AMOUNT</span></div><div class="pr-entry-values"><strong>${field(invoice,'invoiceNumber')}</strong><span>${field(invoice,'poReferenceArea')}</span><span>${esc(displayDate(invoice.invoiceDate))}</span><span>${esc(displayDate(invoice.dueDate))}</span><span>${field(invoice,'paymentTerms')}</span><span>${esc(currency)}</span><strong>${money(currency,invoiceTotal)}</strong></div><div class="pr-breakdown-row">${entryAmounts.map((entry,index)=>`<span><b>${labels[index]}</b><em>${esc(amount(entry))}</em></span>`).join('')}</div></div></section>
