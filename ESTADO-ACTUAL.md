...
- Paquete listo en `Claude outputs/instalacion-oficial/` (no va al repositorio): `vaak-plataforma-oficial.zip` (con `nucleo/config.php` para la base `wwwhpgilatam_vaakoficial`, usuario `wwwhpgilatam_vkof`, origen `https://plataforma.hpgilatam.com`, secreto propio distinto del de prueba), `1-tablas.sql` y `2-primer-administrador.sql`.
