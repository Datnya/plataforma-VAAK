...
En `Claude outputs/instalacion-prueba/` (no se sube al repositorio): `vaak-plataforma.zip` (sitio armado, rutas con `/` para que Linux lo descomprima bien, incluye `nucleo/config.php` con la base `wwwhpgilatam_vaakprueba` y la contraseña por pegar: `PEGA-AQUI-LA-CONTRASEÑA`), `1-tablas.sql` (se importa en phpMyAdmin) y `2-primer-administrador.sql` (usuario temporal `admin.prueba`; su contraseña temporal se le dio a Datnya en el chat y debe cambiarla al entrar). `verificar.php` (en la raíz del sitio) muestra en español qué falta: versión de PHP, extensiones, contraseña, conexión, tablas y administrador. Probado en local de punta a punta.
