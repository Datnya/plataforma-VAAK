...
foreach (array('pdo_mysql', 'mbstring', 'openssl', 'json') as $ext) {
...
if (is_array($config)) {
  ...
  if ($clavePuesta && extension_loaded('pdo_mysql')) {
    try {
      $c = $config['db'];
      $db = new PDO(sprintf('mysql:host=%s;port=%d;dbname=%s;charset=utf8mb4', $c['host'], isset($c['port']) ? (int)$c['port'] : 3306, $c['nombre']), $c['usuario'], $c['clave'], array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
      fila($filas, $bien, true, 'Conexión a la base de datos', 'correcta (MySQL ' . $db->query('SELECT VERSION()')->fetchColumn() . ')');
      $tablas = (int)$db->query("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name LIKE 'vaak\\_%'")->fetchColumn();
      ...
      if ($tablas >= 10) {
        $admins = (int)$db->query("SELECT COUNT(*) FROM vaak_user_company_memberships WHERE role = 'admin' AND status = 'active'")->fetchColumn();
