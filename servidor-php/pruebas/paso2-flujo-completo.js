...
module.exports = async (run) => {
  ...
  informe.flujo = await run.eval(`
    ...
    const spec=[...f.elements.namedItem('itemSpec0').options].map(o=>o.value).find(Boolean); await set('itemSpec0',spec); await set('itemQuantity0','1'); await set('itemCost0','1234.56'); await set('taxType','IGV'); await set('paymentTerms','Contado'); await set('specifiedBy','ESTUDIO XYZ'); await wait(400);
    // Dos decimales en el formulario -> dos decimales en el impuesto y en el total (24-sep-2026).
    r.decimales={enPantalla:document.getElementById('po-grand-total')?.textContent.trim(), impuesto:f.elements.namedItem('taxAmount')?.value};
    ...
    let st=JSON.parse(localStorage.getItem('vaak-local-v8')); const oc=st.orders.at(-1); r.oc={numero:oc.number, total:oc.amount, impuesto:oc.taxAmount, especificadoPor:oc.specifiedBy};
